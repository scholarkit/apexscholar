...
function PipelineStep({ label, icon, done, active }: Step & { active: boolean }) {
  return (
    <div
      className={`flex items-center gap-2 text-xs transition-all duration-300 ${done ? 'text-emerald-400' : active ? 'text-indigo-500' : 'text-zinc-600'}`}
    >
      ...
      >
        ...
        {!meta && !error && (
          <div className="px-3 sm:px-6 py-3 sm:py-4 space-y-5">
            ...
            <p className="text-zinc-600 text-xs pl-8">
              This may take a few seconds for large PDFs or DOI lookups.
            </p>
          </div>
        )}
        ...
        {error && (
          <div className="px-6 py-8 flex flex-col items-center gap-3 text-center">
            ...
            <button
              onClick={onClose}
              className="mt-2 px-4 py-2 bg-white/5 hover:bg-white/10 text-white rounded-xl text-sm"
            >
              Close
            </button>
          </div>
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="space-y-1 flex-1 min-w-0">
                  ...
                  <div className="flex flex-wrap gap-2 text-xs text-zinc-500">
                    ...
                    {meta.doi && (
                      <span className="font-mono truncate max-w-[180px]">• {meta.doi}</span>
                    )}
                  </div>
                </div>
                {badge && (
                  <span
                    className={`flex-shrink-0 flex items-center gap-1.5 text-xs font-medium px-2.5 py-1 rounded-full border ${badge.color}`}
                  >
                    {badge.icon}
                    {badge.label}
                  </span>
