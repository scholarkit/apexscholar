  ...
  const handleSend = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      supermemory.addMemory(
        `[resource] chat: file=${resource.name}, userMessageLength=${userMessage.length}, aiResponseLength=${fullResponse.length}`,
        {
          module: 'resource',
          action: 'chat',
          ...
          {loadingHistory ? (
            ...
            <div className="h-full flex flex-col items-center justify-center text-center px-8">
              ...
              <p className="text-xs text-zinc-500 max-w-[280px]">
                I've indexed **{resource.name}**. Ask me questions about its content, purpose, or
                data.
              </p>
            </div>
          ) : (
                  ...
                  <div
                    className={`w-8 h-8 rounded-xl flex items-center justify-center flex-shrink-0 ${
                      m.role === 'user'
                        ...
                        : 'bg-[var(--color-surface)] border border-[var(--color-border)] text-zinc-400'
                    }`}
                  >
