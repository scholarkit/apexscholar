...
export const journalService = {
  ...
  async updateEntry(id: string, patch: Partial<JournalEntry>): Promise<JournalEntry> {
    ...
    supermemory.addMemory(
      `[journal] update_entry: entryId=${id}, updatedFields=${Object.keys(patch).join(', ')}`,
      {
        module: 'journal',
        action: 'update_entry',
        entryId: id,
  ...
  async createInsight(insight: Omit<JournalInsight, 'id'>): Promise<JournalInsight> {
    ...
    supermemory.addMemory(
      `[journal] create_insight: projectId=${insight.projectId || 'none'}, contentPreview=${insight.content.substring(0, 100)}${insight.content.length > 100 ? '...' : ''}`,
      {
        module: 'journal',
