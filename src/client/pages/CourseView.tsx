  ...
  if (!course) {
    return (
      <div className="flex flex-col items-center justify-center py-32 text-center">
        ...
        <Link to="/learn" className="mt-4 text-indigo-400 text-sm hover:text-indigo-300">
          ← Back to Learn
        </Link>
      </div>
    );
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <nav className="flex items-center gap-2 text-xs text-zinc-500">
        <Link to="/learn" className="hover:text-zinc-300 transition-colors">
          Learn
        </Link>
      ...
      <div className="bg-[var(--color-surface)]/40 border border-[var(--color-border)] rounded-xl p-6 sm:p-8">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex flex-wrap items-center gap-2 mb-2">
              ...
              {course.tags.map((t) => (
                <span
                  key={t}
                  className="text-[11px] px-2.5 py-1 rounded-full bg-white/5 border border-[var(--color-border)] text-zinc-400"
                >
                  {t}
                </span>
              ))}
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between text-xs">
            ...
            <span
              className={`font-bold text-sm ${pct === 100 ? 'text-emerald-400' : 'text-indigo-400'}`}
            >
              {pct}%
            </span>
          </div>
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        <p
                          className={`text-sm font-medium truncate ${isDone ? 'text-zinc-400' : 'text-white'}`}
                        >
                          {lesson.title}
                        </p>
                      </div>
                    ...
                    <div className="flex items-center gap-3 flex-shrink-0 text-zinc-600">
                      ...
                      <ChevronRight
                        className={`w-4 h-4 transition-colors ${isDone ? 'text-emerald-600' : 'group-hover:text-indigo-400'}`}
                      />
                    </div>
                  </Link>
                ) : (
