...
async function searchPubmed(query: string): Promise<Paper[]> {
  try {
    ...
    return articles.map((article) => {
      ...
      const title =
        article.querySelector('ArticleTitle')?.textContent?.replace(/\s+/g, ' ').trim() ||
        'Untitled';
      ...
      const authors = Array.from(authorEls)
        .map((a) => {
          ...
          const format =
            ...
            a.querySelector('Initials')?.textContent ||
            '';
      ...
      const year =
        ...
        article.querySelector('DateRevised Year')?.textContent ||
        '';
...
export function upsertNode(
  ...
  let node: any = graph.nodes.find((n) => n.node_type === type && normalize(n.label) === key);
...
export function connect(
  ...
  const exists = graph.edges.find(
    (e) => e.source_id === source.id && e.target_id === target.id && e.relation === relation
  );
...
export function updateGraphFromInsight(graph: KGGraph, insight: PaperInsight) {
  const entities = extractEntities(insight);

  const problemNode =
    entities.problems.length > 0
...
export function detectGaps(graph: KGGraph): ResearchGap[] {
  ...
  methods.forEach((m) => {
    domains.forEach((d) => {
      const exists = graph.edges.find(
        (e) => e.source_id === m.id && e.target_id === d.id && e.relation === 'applies_to'
      );
        ...
        <div className="flex items-center gap-2">
          {isEditing ? (
            <button
              ...
              className="text-xs px-2 py-1 bg-emerald-500/20 text-emerald-400 rounded-xl hover:bg-emerald-500/30 transition-colors"
            >
              Save
            </button>
          ) : (
      ...
      <div className="p-4 space-y-4 max-h-96 overflow-y-auto custom-scrollbar text-sm">
        ...
        <div className="flex gap-4 mb-4 border-b    border-[var(--color-border)] pb-4">
          <div className="flex-1">
            ...
            {isEditing ? (
              <input
                value={localInsight.domain}
                onChange={(e) => handleFieldChange('domain', e.target.value)}
                className="w-full bg-[var(--color-surface)] border border-[var(--color-border)] p-1 text-xs text-white rounded outline-none"
              />
            ) : (
        ...
        {(['problem', 'method'] as const).map((field) => (
          <div key={field}>
            ...
            {isEditing ? (
              <textarea
                value={localInsight[field] as string}
                onChange={(e) => handleFieldChange(field, e.target.value)}
                className="w-full bg-[var(--color-surface)] border border-[var(--color-border)] p-2 text-xs text-white rounded outline-none min-h-[60px]"
              />
            ) : (
              <p className="text-white bg-[var(--color-border)] p-2 rounded-xl leading-relaxed text-xs">
                {(localInsight[field] as string) || '—'}
              </p>
            )}
          </div>
        ))}
    ...
    <div className="flex flex-col items-center justify-center py-12 sm:py-20 animate-in fade-in zoom-in duration-500">
      <div className="text-center mb-10">
        ...
        <p className="text-zinc-500 text-sm max-w-md mx-auto">
          Connecting to global repositories and extracting relevant papers...
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 w-full max-w-4xl px-4">
        {statuses.map((s) => (
          ...
          >
            ...
            {s.status === 'loading' && <Loader2 className="w-4 h-4 text-indigo-400 animate-spin" />}
          </div>
        ))}
      </div>
    </div>
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (contentType && contentType.includes('application/json')) {
            ...
            console.warn(
              'PDF resolution returned non-JSON response (likely an SPA fallback). Check if server requires restart.'
            );
          }
        }
      } catch (e) {
        ...
        <div>
          ...
          {paper.abstract.length > 200 && (
            <button
              ...
              className="text-xs text-indigo-500 hover:text-indigo-300 mt-1"
            >
          ...
          <>
            {insight ? (
              <button
                ...
                className={`flex items-center gap-1.5 text-xs px-1 sm:px-2 py-1 sm:py-1.5 rounded-xl transition-colors ${isInsightOpen ? 'text-white bg-indigo-500' : 'text-indigo-500 hover:text-indigo-300 hover:bg-indigo-400/10'}`}
              >
                <Lightbulb
                  className={
                    isInsightOpen ? 'text-amber-200 w-3.5 h-3.5' : 'text-amber-400 w-3.5 h-3.5'
                  }
                />
        ...
        await Promise.all(
          activeSources.map(async (src) => {
            try {
              ...
              setSourceStatuses((prev) =>
                prev.map((s) => (s.id === src.id ? { ...s, status: 'error', count: 0 } : s))
              );
            }
          })
        );
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-center">
        ...
        <p className="text-zinc-500 mb-8 max-w-sm">
          You must select or create a project before accessing the Exploration module.
        </p>
        <button
    ...
    <div className="space-y-8 animate-in fade-in duration-500 pb-32 lg:pb-8">
      ...
      <header className="relative">
        ...
        <div className="flex items-center gap-3 mb-1">
          ...
          <span className="px-2 py-0.5 text-xs font-semibold bg-indigo-500/15 text-indigo-500 border border-indigo-500/25 rounded-full">
            Beta
          </span>
        </div>
        <p className="text-base text-[var(--color-text-muted)] max-w-2xl leading-relaxed">
          Aggregate research from <span className="text-[var(--color-text-faint)]">arXiv</span>,{' '}
          <span className="text-[var(--color-text-faint)]">OpenAlex</span>,{' '}
          <span className="text-[var(--color-text-faint)]">Google Scholar</span>,{' '}
          <span className="text-[var(--color-text-faint)]">CrossRef</span> and{' '}
          <span className="text-[var(--color-text-faint)]">Semantic Scholar</span> instantly.
        </p>
      </header>
      ...
      <div className="flex flex-wrap gap-1 bg-[var(--color-surface)] border border-[var(--color-border)] p-1 rounded-xl w-fit">
        {(['search', 'saved', 'graph'] as const).map((tab) => (
          <button
            ...
            className={`w-full sm:w-fit px-4 py-2 rounded-xl text-sm font-medium transition-all capitalize flex items-center gap-2 ${
              activeTab === tab
          ...
          >
            ...
            {tab === 'search' ? 'Discover' : tab === 'saved' ? 'Knowledge Base' : 'Reasoning Graph'}
            {tab === 'saved' && savedPapers.length > 0 && (
              <span className="ml-1 px-1.5 py-0.5 text-xs bg-[var(--color-surface)] text-indigo-500 rounded-full">
                {savedPapers.length}
              </span>
            )}
        ...
        <div className="space-y-6">
          ...
          <form onSubmit={handleSearch} className="flex flex-col sm:flex-row gap-3">
            ...
            <div className="relative group min-w-[160px]">
              ...
              >
                <option value="all" className="bg-[var(--color-surface)]">
                  All Sources
                </option>
                <option value="arxiv" className="bg-[var(--color-surface)]">
                  arXiv
                </option>
                <option value="openalex" className="bg-[var(--color-surface)]">
                  OpenAlex
                </option>
                <option value="semanticscholar" className="bg-[var(--color-surface)]">
                  Semantic Scholar
                </option>
                <option value="googlescholar" className="bg-[var(--color-surface)]">
                  Google Scholar
                </option>
                <option value="pubmed" className="bg-[var(--color-surface)]">
                  PubMed
                </option>
                <option value="crossref" className="bg-[var(--color-surface)]">
                  CrossRef
                </option>
              </select>
          ...
          {loading && (
            <SourceScanner
              statuses={sourceStatuses.filter((s) => source === 'all' || s.id === source)}
            />
          )}
              ...
              {!loadingRecs && recommendations.length === 0 && (
                <div className="text-center py-20 border border-[var(--color-border)] rounded-2xl bg-gradient-to-b from-white/[0.02] to-transparent backdrop-blur-sm relative overflow-hidden group">
                  ...
                  <p className="text-sm text-zinc-500 max-w-md mx-auto leading-relaxed px-6">
                    Enter a topic, DOI, or author name above to scan humanity's collective knowledge
                    across all major scientific repositories.
                  </p>
                  <div className="mt-8 flex justify-center gap-4 text-[10px] font-bold uppercase tracking-widest text-zinc-600">
                    <span className="px-3 py-1 border border-[var(--color-border)] rounded-full">
                      300M+ Papers
                    </span>
                    <span className="px-3 py-1 border border-[var(--color-border)] rounded-full">
                      Real-time Insights
                ...
                <div className="space-y-4 animate-pulse">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {[1, 2].map((i) => (
                      <div
                        key={i}
                        className="h-48 bg-[var(--color-surface)] border border-[var(--color-border)] rounded-xl"
                      ></div>
                    ))}
                  </div>
                </div>
          ...
          {savedPapers.length === 0 ? (
            <div className="text-center py-20 border border-dashed border-[var(--color-border)] rounded-xl bg-[var(--color-surface)]/20">
              ...
              <p className="text-zinc-500 text-sm max-w-sm mx-auto">
                Search for papers and click <strong>Import</strong> to add them here.
              </p>
              <button
                  ...
                  <div className="space-y-6">
                    ...
                    <div>
                      <h4 className="text-xs font-semibold text-zinc-500 uppercase tracking-widest mb-3">
                        Papers in Knowledge Base ({selectedNode.paper_ids.length})
                      </h4>
                      <div className="space-y-3">
                        {selectedNode.paper_ids.map((pid) => {
                          ...
                          return (
                            <div
                              key={pid}
                              className="bg-[var(--color-surface)]/80 border border-[var(--color-border)] p-3 rounded-xl"
                            >
      ...
      {/* Citation Modal */}
      {citingPaper && (
        <QuickCiteModal meta={paperToMeta(citingPaper)} onClose={() => setCitingPaper(null)} />
      )}
    </div>
  );
}
