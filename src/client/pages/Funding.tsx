...
import { fundingService, type Grant } from '../lib/funding';
      ...
      <header className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        ...
        <div>
          ...
          <p className="text-base text-zinc-400">
            Manage proposals, observe deadlines, and track your research funding.
          </p>
        </div>
        <button
          onClick={() => {
...
function StatusBadge({ status }: { status: Grant['status'] }) {
  const colors = {
    ...
    drafting: 'bg-indigo-500/10 text-indigo-500 border-indigo-500/20',
...
function GrantCard({
  ...
  const isOverdue = Boolean(
    deadlineDay && deadlineDay < today && !['awarded', 'rejected'].includes(grant.status)
  );
  ...
  return (
    <div className="bg-[var(--color-surface)]/40 border    border-[var(--color-border)] hover:border-[var(--color-border)] hover:bg-[var(--color-surface)]/60 transition-all rounded-xl p-2.5 sm:p-5 flex flex-col gap-4 group relative">
      ...
      <div className="pr-12">
        ...
        <h3 className="text-base font-semibold leading-snug line-clamp-2" title={grant.title}>
          {grant.title}
        </h3>
        ...
        {view === 'requirements' && (
          <div className="space-y-3">
            <div className="flex items-center gap-2 text-sm font-medium text-white mb-3">
              <ListTodo className="w-4 h-4 text-indigo-500" /> Checklist
            </div>
                    ...
                    <div
                      className={cn(
                        ...
                        req.completed
                          ...
                          : 'border-zinc-600 group-hover/req:border-indigo-400'
                      )}
                    >
                    ...
                    <span
                      className={cn(
                        ...
                        req.completed
                          ...
                          : 'text-zinc-300 group-hover/req:text-white transition-colors'
                      )}
                    >
                      {req.description}
                    </span>
            ...
            <div className="space-y-2">
              <div className="flex justify-between text-sm">
                ...
                <span className="font-semibold text-white">
                  ${Number(grant.budget.requested).toLocaleString()}
                </span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-zinc-500">Awarded</span>
                <span className="font-semibold text-emerald-400">
                  ${Number(grant.budget.awarded).toLocaleString()}
                </span>
              </div>
              <div className="flex justify-between text-sm pt-2 border-t    border-[var(--color-border)]">
                ...
                <span className="font-semibold text-amber-400">
                  ${Number(grant.budget.spent).toLocaleString()}
                </span>
              </div>
            </div>
            ...
            {Number(grant.budget.awarded) > 0 && (
              <div className="h-2 w-full bg-zinc-800 rounded-full overflow-hidden flex">
                <div
                  ...
                  style={{
                    width: `${Math.min((Number(grant.budget.spent) / Number(grant.budget.awarded)) * 100, 100)}%`,
                  }}
                  title="Spent"
                />
                <div
...
function GrantFormModal({
  ...
  const addReq = () => {
    ...
    setFormData((prev) => ({
      ...
      requirements: [
        ...(prev.requirements || []),
        { id: crypto.randomUUID(), description: newReq.trim(), completed: false },
      ],
    }));
    setNewReq('');
  };
      ...
      <div className="bg-[var(--color-surface)] border border-[var(--color-border)] rounded-xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col">
        ...
        >
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-xs font-medium text-zinc-400 mb-1">
                  Document Link (e.g. Google Docs)
                </label>
                <input
                  type="url"
                  value={formData.document_url || ''}
                  onChange={(e) => setFormData({ ...formData, document_url: e.target.value })}
                  className="w-full bg-[var(--color-surface)] border border-[var(--color-border)] rounded-xl px-3 py-2 focus:outline-none focus:border-indigo-500/50 placeholder:text-zinc-600"
                  placeholder="https://..."
                />
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setFormData({
                      ...formData,
                      budget: { ...formData.budget!, requested: Number(e.target.value) },
                    })
                  }
                  className="w-full bg-[var(--color-surface)] border border-[var(--color-border)] rounded-xl px-3 py-2 focus:outline-none focus:border-indigo-500/50"
                />
              ...
              <div>
                ...
                <input
                  type="number"
                  min="0"
                  value={formData.budget?.awarded || ''}
                  onChange={(e) =>
                    setFormData({
                      ...formData,
                      budget: { ...formData.budget!, awarded: Number(e.target.value) },
                    })
                  }
                  className="w-full bg-[var(--color-surface)] border border-[var(--color-border)] rounded-xl px-3 py-2 focus:outline-none focus:border-indigo-500/50"
                />
          ...
          <div className="space-y-4">
            ...
            <div className="p-4 bg-[var(--color-surface)]/50 border    border-[var(--color-border)] rounded-xl space-y-3">
              {formData.requirements?.map((req, i) => (
                <div
                  key={req.id}
                  className="flex items-center gap-2 p-2 bg-[var(--color-surface)] border    border-[var(--color-border)] rounded-xl"
                >
        ...
        <div className="p-5 border-t border-[var(--color-border)] flex justify-end gap-3 bg-[var(--color-surface)]/30">
          <button
            type="button"
            onClick={onClose}
            className="px-4 py-2 font-medium text-sm text-zinc-400 hover:text-white transition-colors"
          >
            Cancel
          </button>
          <button
            type="submit"
            form="grant-form"
            className="px-5 py-2 bg-indigo-500 hover:bg-indigo-600 text-white rounded-xl text-sm font-medium transition-colors  "
          >
