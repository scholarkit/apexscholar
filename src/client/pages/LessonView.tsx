...
export default function LessonView() {
  ...
  const nextLesson =
    lessonIndex < (course?.lessons.length ?? 0) - 1 ? course!.lessons[lessonIndex + 1] : null;
  ...
  const handleListen = async () => {
    ...
    try {
      ...
      const speech = await ai.txt2speech(`Lesson: ${lesson.title}. ${plainText}`, {
        provider: 'elevenlabs',
  ...
  return (
    <div className="animate-in fade-in duration-500 pb-32 sm:pb-8">
      ...
      <nav className="flex items-center gap-2 text-xs text-zinc-500 mb-6">
        ...
        <Link to={`/learn/${course.id}`} className="hover:text-zinc-300 transition-colors">
          {course.title}
        </Link>
        <ChevronRight className="w-3.5 h-3.5" />
        ...
        {lesson.content ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 text-center not-prose">
            ...
            <p className="text-zinc-600 text-sm mt-1">
              Check back shortly — lessons are being added one by one.
            </p>
          </div>
        )}
      </article>
