...
export default function ZoteroImportModal({ onClose, onImport }: ZoteroImportModalProps) {
  ...
  const handleImport = async () => {
    ...
    try {
        ...
        // Map items to Resource schema
        const mappedResources: Resource[] = allItems.map(item => {
            ...
            // Format creators carefully
            let authorString = 'Unknown Author';
            if (data.creators && data.creators.length > 0) {
                authorString = data.creators.map((c: any) => {
                    if (c.name) return c.name;
                    return `${c.firstName || ''} ${c.lastName || ''}`.trim();
                }).join(', ');
            }

            // Create markdown content holding all the tags, abstract and authors nicely.
            const content = `# ${data.title || 'Untitled'}\n\n**Authors:** ${authorString}\n**Year:** ${data.date || 'Unknown'}\n**DOI:** ${data.DOI || 'none'}\n\n**Abstract:**\n${data.abstractNote || 'No abstract available.'}\n\n**Tags:** ${(data.tags || []).map((t:any) => t.tag).join(', ')}`;
            
            return {
                id: `zotero_${data.key}`,
                projectId: activeProject.id,
                name: `${data.title || 'Untitled'} (Zotero)`,
                type: 'text/markdown',
                path: `research-dashboard/uploads/zotero_${data.key}.md`, // we will mock standard upload path
      ...
      >
        ...
        <div className="p-6 space-y-6">
          <p className="text-sm text-zinc-400">
            Select which library or collection to import and sync into this workspace.
          </p>
          ...
          <div className="space-y-4">
            <div>
              ...
              <div className="p-2 bg-black/40 border border-neutral-800 rounded-xl space-y-1 max-h-[240px] overflow-y-auto">
                {isLoading ? (
