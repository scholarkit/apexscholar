...
exploreRouter.get('/graph', requireAuth, async (req, res) => {
  try {
    ...
    const [{ data: nodes, error: nodesError }, { data: edges, error: edgesError }] =
      await Promise.all([
        supabaseAdmin.from('knowledge_graph_nodes').select('*').eq('graph_id', graph.id),
        supabaseAdmin.from('knowledge_graph_edges').select('*').eq('graph_id', graph.id),
      ]);
...
exploreRouter.post('/graph/clear', requireAuth, async (req, res) => {
  try {
    ...
    await supabaseAdmin.from('knowledge_graph_edges').delete().eq('graph_id', graph_id);

    // Delete all nodes
    await supabaseAdmin.from('knowledge_graph_nodes').delete().eq('graph_id', graph_id);
